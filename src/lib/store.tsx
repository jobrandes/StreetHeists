...
function normalizeProgress(raw?: Partial<CaseProgress> | null): CaseProgress {
  return {
    ...
    clueLinks: Array.isArray(raw?.clueLinks)
      ...
      : [],
    securedChainIds: Array.isArray(raw?.securedChainIds)
      ? raw.securedChainIds.filter((id): id is string => typeof id === "string")
      : [],
...
type Store = {
  ...
  addClueLink: (
    ...
    a: string,
    b: string,
  ) => { link: ClueLink | null; sound: boolean; message: string; unlockedChainIds: string[]; missReason?: string; hint?: string };
      ...
      addClueLink(caseId, a, b) {
        ...
        setState((current) => ({
          ...
          progressByCase: patchCase(current.progressByCase, caseId, (p) => {
            ...
            const secured = new Set([...(p.securedChainIds ?? []), ...grade.unlockedChainIds]);
            return {
              ...p,
              clueLinks: [...(p.clueLinks ?? []), saved],
              securedChainIds: [...secured],
            };
          }),
        }));
