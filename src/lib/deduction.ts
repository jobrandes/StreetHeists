...
/** Chains that still need a Contradiction desk / Compare find before yarn can unlock them. */
export function chainsBlockedOnCompare(
  caseFile: CaseFile,
  progress: CaseProgress,
): DeductionChain[] {
  return (caseFile.deductionChains ?? []).filter((chain) => {
    if (!chain.unlockOnContradictionId) return false;
    if (progress.foundContradictionIds.includes(chain.unlockOnContradictionId)) {
      return false;
    }
    return true;
 ...
 * Only newly unlocked chains count toward Accuse (failed yarn never does).
 */
export function gradeClueLink(
  ...
  if (matching.length === 0) {
    return {
      sound: false,
      unlockedChainIds: [],
      message: "No chain there.",
      missReason: "These don't share a time, person, or route.",
  ...
  const compareBlocked = matching.filter((chain) => {
    if (!chain.unlockOnContradictionId) return false;
    if (progress.foundContradictionIds.includes(chain.unlockOnContradictionId)) {
      return false;
    }
    return correctPairsSatisfied(chain, nextLinks);
  });
  if (compareBlocked.length > 0) {
    return {
      sound: true,
      unlockedChainIds: [],
      message:
        "Right pair — but this chain needs Compare first. Spot the contradiction in the Locker (Contradiction desk), then the yellow note unlocks.",
      hint: "Some Last Toast–style chains stay locked until Compare finds the clash.",
    };
  }
