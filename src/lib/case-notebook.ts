...
export function buildCaseNotebook(
  ...
  const clueLines = opened.map((item) => ({
    label: item.title,
    body: isDeductionUnlocked(caseFile, progress, item)
      ? item.deduction
      : "Takeaway locked — string this clue on the corkboard.",
  }));

  const chainLines = unlockedDeductionChains(caseFile, progress).map((chain) => ({
    label: chain.title,
    body: chain.insight,
  }));
  ...
  return [
    ...
    {
      id: "chains",
      title: "Deduction chains unlocked",
      empty: "Link clues on the corkboard — chain insights file here when they unlock.",
      lines: chainLines,
    },
    {
      id: "contradictions",
