...
export function ShareCardC({ caseFile, verdict, alias }: { caseFile: CaseFile; verdict: Verdict; alias: string }) {
  const cleanSolve = verdict.wrongAttempts === 0;
  ...
  return (
    <div className="h-[240px] w-[480px] overflow-hidden border border-[#C9A227]/60 bg-[#0B0B0C] p-4 text-[#F2F0EA]">
      <div className="flex items-center justify-between">
        ...
        <p className="font-display text-lg tracking-[0.14em] text-[#C9A227] uppercase">{`Case closed · ${String(caseFile.number).padStart(2, "0")}`}</p>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between">
        <p className="font-serif text-xl font-bold">{caseFile.title}</p>
        <p className="text-xs text-[#F2F0EA]">{`${alias} · ${formatElapsed(verdict.elapsedMs)} · ${cleanSolve ? "CLEAN SOLVE" : "SOLVED"}`}</p>
      </div>
    </div>
  );
}

export function ShareExports({ caseFile, verdict, alias }: { caseFile: CaseFile; verdict: Verdict; alias: string }) {
  ...
  async function exportCard(which: "A" | "C") {
    ...
    try {
      ...
      const file = new File([blob], `street-heists-card-${which.toLowerCase()}.png`, { type: "image/png" });
      if (navigator.canShare?.({ files: [file] })) {
        await navigator.share({ files: [file], title: `${caseFile.title} · Card ${which}` });
      } else {
  ...
  return (
    <div className="space-y-7">
      <div className="pointer-events-none fixed -left-[1400px] top-0 space-y-8">
        <div ref={cardA}><ShareCardA caseFile={caseFile} verdict={verdict} alias={alias} /></div>
        <div ref={cardC}><ShareCardC caseFile={caseFile} verdict={verdict} alias={alias} /></div>
      </div>
      <section>
        <p className="mb-2 font-display text-[11px] font-bold tracking-[0.2em] text-gold uppercase">Card A · Case poster</p>
        <div className="h-[464px] overflow-hidden rounded-lg bg-[#0B0B0C]"><div className="origin-top-left scale-[0.86]"><ShareCardA caseFile={caseFile} verdict={verdict} alias={alias} /></div></div>
      ...
      <section>
        ...
        <div className="h-[175px] overflow-hidden rounded-lg bg-[#0B0B0C]"><div className="origin-top-left scale-[0.72]"><ShareCardC caseFile={caseFile} verdict={verdict} alias={alias} /></div></div>
