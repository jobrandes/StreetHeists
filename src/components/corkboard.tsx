...
export function MosaicCorkboard({
  ...
  onLink: (
    a: string,
    b: string,
  ) => { sound: boolean; message: string; unlockedChainIds: string[]; missReason?: string; hint?: string };
  onClearChain: () => void;
  ...
  /** Tutorial rails: once filed, keep the example pair on the board. */
  useEffect(() => {
    if (!examplePair || !exampleFiled) return;
  ...
  }, [examplePair, exampleFiled, pinnedIds, onTogglePin]);
  ...
  function tryLink(a: string, b: string) {
    ...
    if (!result.sound && (result.missReason || result.hint)) {
      setToast(
        [result.message, result.missReason ? `Why: ${result.missReason}` : null, result.hint ? `Try: ${result.hint}` : null]
          .filter(Boolean)
          .join(" "),
      );
    } else {
  ...
  return (
    <section className="space-y-3">
      ...
      <nav
        aria-label="How to link clues"
        className="grid grid-cols-3 gap-1 rounded-xl border border-[#C4A574]/45 bg-[#FFF8EE] p-1.5"
      >
        {(
          [
            { id: "open", label: "1 · Open a pin", on: linkPick.length === 0 && step === "connect" },
            { id: "link", label: "2 · Link to chain", on: linkPick.length === 1 },
            { id: "second", label: "3 · Second clue", on: linkPick.length === 1 },
          ] as const
        ).map((item) => {
          const idle = step !== "connect" && step !== "done";
          const doneStep = step === "done" || (item.id === "open" && linkPick.length >= 1);
          return (
            <div
              key={item.id}
              className={cn(
                "rounded-lg px-2 py-2 text-center font-display text-[10px] font-bold tracking-[0.06em] uppercase",
                item.on && "bg-[#1B2430] text-[#F2F0EA]",
                doneStep && !item.on && "bg-[#E8D4B0] text-ink",
                !item.on && !doneStep && "text-muted",
                idle && "opacity-70",
              )}
            >
              {item.label}
      ...
      <div className="rounded-xl border-2 border-[#1B2430]/15 bg-white px-4 py-3 shadow-[0_2px_0_rgba(27,36,48,0.06)]">
        ...
        {step === "pin" ? (
          <>
            ...
            {examplePair ? (
              <p className="mt-1 text-base font-semibold leading-snug text-ink">
                Tutorial rails are labeled on the board:{" "}
                <span className="underline">{examplePair.a.title}</span> and{" "}
                <span className="underline">{examplePair.b.title}</span>. Hang both if they are not up yet.
              </p>
            ) : (
              <p className="mt-1 text-base font-semibold leading-snug text-ink">
        ...
        {step === "connect" ? (
          <>
            ...
            {examplePair && !exampleLinked ? (
              <>
                <p className="mt-1 text-base font-semibold leading-snug text-ink">
                  Forced tutorial — connect the two Rails:
                </p>
                <div className="mt-3 rounded-lg border-2 border-dashed border-[#C62828]/45 bg-[#FFF5F5] px-3 py-3">
                  ...
                  <p className="mt-2 text-sm text-muted">
                    Open a Rail pin → link to chain → choose the other Rail. Or tap Connect for me.
                  </p>
                </div>
              </>
            ) : (
              <>
                <p className="mt-1 text-base font-semibold leading-snug text-ink">
                  {linkPick.length === 0
                    ? "Open a pin, then choose a second clue that belongs on the same chain."
                    : "Link to chain — now choose the second clue."}
                </p>
                <p className="mt-1 text-sm text-muted">
                  Right pair → chain counts. Wrong pair → yarn does not stick (progress unchanged).
                </p>
              </>
            )}
          </>
        ...
        {step === "done" ? (
          <>
            ...
            <p className="mt-1 text-base font-semibold leading-snug text-ink">
              {examplePair
                ? `Chain ${stickiesHave}/${stickiesNeeded || 1} — Accuse unlocked. Nice work.`
                : `Chains ${stickiesHave}/${stickiesNeeded || stickiesHave} — Accuse is open.`}
            </p>
          </>
        ) : null}
        <p className="mt-3 text-sm font-semibold text-ink">
          Chains {stickiesHave}/{stickiesNeeded || Math.max(stickiesHave, 1)}
          {ready ? " · Accuse unlocked" : " · keep linking sound pairs"}
        </p>
      </div>
      ...
      {examplePair ? (
        <FirstUseTip
          ...
          text={`Tutorial tip: hang ${examplePair.a.title} + ${examplePair.b.title}, then Open a pin → Link → choose the other Rail (or Connect for me). Chain 1/1 unlocks Accuse.`}
        />
      ) : null}
      ...
      {step === "connect" ? (
        <div className="space-y-2">
          ...
          <div className="flex h-14 items-center justify-center gap-2 rounded-xl border-2 border-dashed border-[#C62828]/50 bg-[#F8D7D7]/45 font-display text-sm font-bold tracking-[0.1em] text-ink uppercase">
            ...
            {linkPick.length === 0
              ? examplePair
                ? "Or tap the two Rail-tagged photos"
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted">
          ...
          {links.length > 0 ? (
            <button
              ...
              onClick={() => {
                onClearChain();
                setToast("Yarn cleared — pins and chains stay.");
                setLinkPick([]);
              }}
            >
              Clear yarn
